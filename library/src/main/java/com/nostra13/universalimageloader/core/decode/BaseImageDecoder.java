...
public class BaseImageDecoder implements ImageDecoder {
	private static final String LOG_TAG = "BaseImageDecoder";
	...
	protected ExifInfo defineExifOrientation(String imageUri, String mimeType) {
		...
		if ("image/jpeg".equalsIgnoreCase(mimeType)
				&& (Scheme.ofUri(imageUri) == Scheme.FILE || Scheme
						.ofUri(imageUri) == Scheme.LOCAL)) {
			try {
				ExifInterface exif = null;
				if (Scheme.ofUri(imageUri) == Scheme.FILE)
					exif = new ExifInterface(Scheme.FILE.crop(imageUri));
				else
					exif = new ExifInterface(Scheme.LOCAL.crop(imageUri));
	...
	protected Bitmap decodeStream(InputStream imageStream,
			Options decodingOptions, String path) throws IOException {
		try {
			ByteArrayOutputStream os = new ByteArrayOutputStream();
			byte[] bytes = new byte[BUFFER_SIZE];
			while (imageStream.read(bytes) != -1) {
				os.write(bytes);
			}
			bytes = os.toByteArray();
			Log.d(LOG_TAG, bytes.toString());
