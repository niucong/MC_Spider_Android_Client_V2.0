 ...
 */
final class LoadAndDisplayImageTaskWithProgress extends
		BaseLoadAndDisplayImageTask {
    private static final String LOG_TAG="LoadAndDisplayImageTaskeWithProgress";
	...
	public LoadAndDisplayImageTaskWithProgress(ImageLoaderEngine engine,
		...
		try {
			...
			if (bitmap == null || bitmap.getWidth() <= 0
				...
				String imageUriForDecoding = options.isCacheOnDisc() ? tryCacheImageOnDisc(imageFile)
						: uri;
				L.d(LOG_TAG, "imageUriForDecoding:"+imageUriForDecoding);
				if (!checkTaskIsNotActual()) {
					bitmap = decodeImage(imageUriForDecoding);
					if (bitmap == null || bitmap.getWidth() <= 0
							|| bitmap.getHeight() <= 0) {
						fireImageLoadingFailedEvent(FailType.DECODING_ERROR,
								null);
					}
				}
			}
	...
	protected void downloadImage(File targetFile) throws IOException {
		InputStream is = getDownloader().getStream(uri,
				options.getExtraForDownloader(), listener);
		try {
			OutputStream os = new BufferedOutputStream(new FileOutputStream(
					targetFile), BUFFER_SIZE);
			listener.onStartProgress();
			try {
				IoUtils.copyStream(is, os, listener,targetFile.getAbsolutePath());
			} catch(Exception e){
				e.printStackTrace();
			}finally {
				IoUtils.closeSilently(os);
