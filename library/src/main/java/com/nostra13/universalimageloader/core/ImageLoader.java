...
public class ImageLoader {
	...
	public void displayImage(String uri, ImageView imageView) {
		displayImage(uri, imageView, null, null, LoadAndDisplay.DEFAULT, null);
	}
	...
	public void displayImage(String uri, ImageView imageView,
		...
		displayImage(uri, imageView, options, null, LoadAndDisplay.DEFAULT,
				null);
	}
	...
	public void displayImage(String uri, ImageView imageView,
		...
		displayImage(uri, imageView, null, listener, LoadAndDisplay.DEFAULT,
				null);
	}
	...
	// BaseLoadAndDisplayImageTask displayTask = new LoadAndDisplayImageTask(
	 ...
	 *            {@linkplain ImageLoadingListener Listener} for image loading
	 *            process. Listener fires events on UI thread.
	 * @param loadAndDisplay
	 *            {@linkplain LoadAndDisplay loadAndDisplay} the type for image
	 *            load and display Task
	 * @param context
	 *            {@linkplain Context context} the context from app
	 * @throws IllegalStateException
	 *             if {@link #init(ImageLoaderConfiguration)} method wasn't
	 *             called before
	 * @throws IllegalArgumentException
	...
	public void displayImage(String uri, ImageView imageView,
			DisplayImageOptions options, ImageLoadingListener listener,
			LoadAndDisplay loadAndDisplay, Context context) {
		checkConfiguration();
		if (imageView == null) {
			throw new IllegalArgumentException(ERROR_WRONG_ARGUMENTS);
		}
		...
		if (bmp != null && !bmp.isRecycled()) {
			...
			ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri,
					imageView, targetSize, memoryCacheKey, options, listener,
					engine.getLockForUri(uri));
			BaseLoadAndDisplayImageTask mDisplayTask = null;
			switch (loadAndDisplay) {
			case DEFAULT:
				mDisplayTask = new LoadAndDisplayImageTask(engine,
						imageLoadingInfo, options.getHandler());
				break;
			case LOCAL:
				mDisplayTask = new LoadLocalAndDisplayImageTask(engine,
						imageLoadingInfo, options.getHandler());
				break;
			case LOCALTHUMBNAIL:
				mDisplayTask = new LoadLocalThumbnailAndDisplayImageTask(
						configuration.context, engine, imageLoadingInfo, options.getHandler());
				break;
			case PROGRESS:
				mDisplayTask = new LoadAndDisplayImageTaskWithProgress(engine,
						imageLoadingInfo, options.getHandler());
				break;
			default:
				break;
			}
			engine.submit(mDisplayTask);
		}
	}
	 ...
	 * callback will be fired with {@link FailReason#NETWORK_DENIED}
	 * 
