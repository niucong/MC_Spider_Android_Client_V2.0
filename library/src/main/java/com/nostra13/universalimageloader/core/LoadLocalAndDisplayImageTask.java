 ...
 */
final class LoadLocalAndDisplayImageTask extends BaseLoadAndDisplayImageTask {
    private static final String LOG_TAG="LoadLocalAndDisplayImageTask";
	...
	public LoadLocalAndDisplayImageTask(ImageLoaderEngine engine,
	...
	public void run() {
		...
		try {
			if (checkTaskIsNotActual())
				return;
             Log.d(LOG_TAG, "memoryCacheKey:"+memoryCacheKey);
	...
	protected Bitmap tryLoadBitmap() {
		...
		try {
			if (imageFile.exists()) {
				...
				bitmap = decodeImage("local://"+imageFile
						.getAbsolutePath());
			}
