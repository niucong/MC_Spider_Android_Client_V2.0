...
import com.datacomo.mc.spider.android.bean.MyQMessageBean.LoadQuuMessageInfo;
...
public class QChatAdapter extends BaseAdapter {
	private final String TAG = "QChatAdapter";

	private Context c;
	private ArrayList<MyQMessageBean> chatInfos;
	...
	public QChatAdapter(Context context, ArrayList<MyQMessageBean> infos,
			ListView lv, String myId, String friendId) {
		c = context;
		chatInfos = infos;
		res = c.getResources();
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		final MyQMessageBean myBean = chatInfos.get(position);
		...
		final MessageBean chatBean = myBean.getMsgBean();

		if (c instanceof QChatActivity)
			((QChatActivity) c).refreshNewNum(chatBean.getMessageId());
		...
		if (null == createTime) {
			...
			date = DateTimeUtil.aTimeFormat(DateTimeUtil.getLongTime(chatBean
					.getCreateTime()));
		}
		...
		ArrayList<MessageResourceInfo> data = (ArrayList<MessageResourceInfo>) chatBean
				.getMessageResourceInfoList();
		int senderId = chatBean.getSenderId();
		final String senderName = chatBean.getSenderName();
		MemberHeadBean head = chatBean.getSenderHead();
		String head_img = ThumbnailImageUrl.getThumbnailHeadUrl(
				head.getHeadUrl() + head.getHeadPath(),
				HeadSizeEnum.ONE_HUNDRED_AND_TWENTY);
		boolean right = myId.trim().equals(senderId + "");
		setHeadName(right, holder, position, head_img, senderId, senderName);
		...
		for (final MessageResourceInfo bean : data) {
			...
			final LoadQuuMessageInfo infoSet = new LoadQuuMessageInfo() {
				...
				@Override
				public void setMessage(MyQMessageBean bean) {
			...
			if ("OBJ_VOICE".equals(oType)) {
				final String fileUrl = bean.getObjectUrl()
						+ bean.getObjectPath();
				float vLong = bean.getObjectBak1();
				...
				if (right) {
					voice.findViewById(R.id.chat_voice_left).setPadding(
							(int) (contentWidth * (vLong / maxLong)), 0, 0, 0);
					...
					voice.findViewById(R.id.chat_voice_right).setPadding(0, 0,
							(int) (contentWidth * (vLong / maxLong)), 0);
				...
				if (!isSending) {
					holder.ll.setOnLongClickListener(new OnLongClickListener() {
						...
						public boolean onLongClick(View v) {
							if (isSuccess) {
								showDialog(myBean, "分享语音", 2, null, fileUrl);
							} else {
				...
				if (!isSuccess) {
					...
					String img_url = ThumbnailImageUrl.getThumbnailImageUrl(
							url, ImageSizeEnum.THREE_HUNDRED);
				...
				if (!isSending) {
					holder.ll.setOnLongClickListener(new OnLongClickListener() {
						...
						public boolean onLongClick(View v) {
							if (isSuccess) {
								showDialog(myBean, "分享照片", 1, img, url);
							} else {
								showFailLongDialog(myBean, infoSet);
							}
							return false;
				...
				if (messageContent == null || messageContent.equals("")) {
					messageContent = chatBean.getMessageContent();
				}
				// L.i(TAG, "messageContent=" + messageContent);
				...
				if (!isSending) {
					holder.ll.setOnLongClickListener(new OnLongClickListener() {
						...
						public boolean onLongClick(View v) {
							if (isSuccess) {
								showDialog(myBean, content, 0, null, null);
							} else {
								showFailLongDialog(myBean, infoSet);
							}
							return false;
	 ...
	 * 长按处理发送失败消息
	 */
	private void showFailLongDialog(final MyQMessageBean myBean,
						...
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								if (which == 0) {
									...
									QChatAdapter.this.notifyDataSetChanged();
									myBean.sendMsg(infoSet);
								} else {
	 ...
	 * @param senderName
	 */
	private void setHeadName(boolean right, ChatViewHolder holder,
			final int position, String head_img, int senderId,
		...
		if (!right) {
			...
			map_Id.put(position + head_img, String.valueOf(senderId));
			...
			L.d(TAG, "setHeadName headUrlPath=" + headUrlPath);
			head_img = ThumbnailImageUrl.getThumbnailHeadUrl(headUrlPath,
					HeadSizeEnum.ONE_HUNDRED_AND_TWENTY);
			...
			// c, "quuchatadapter_2", new ImageCallback() {
			//
			// @Override
			...
			// }
			// }).getDrawable();
			// holder.rightHead.setImageDrawable(ImageDealUtil.getPosterCorner(
			// headDrawable, 4));
	...
	private void showPhoto(String url, String type) {
		...
		bundle.putString(PhotoGalleryActivity.URL, url);
	 ...
	 *            0：文本 1、照片 2、音频 3、视频
	 */
	private void showDialog(final MyQMessageBean myBean, final String text,
	...
	public void onAlertItemClick(int which, MyQMessageBean myBean, int msgType,
			String text, ImageView shareImg, String shareImageUrl,
			DialogInterface dialog) {
		MessageBean bean = myBean.getMsgBean();
		...
		case 1:// 转发到朋友
			...
			if (msgType == 0) {
				...
				String name = "";
				if (GetDbInfoUtil.getMemberId(c) == senderId) {
					name = "我";
				} else {
					name = senderName;
				}
				...
				b.putLong("l", mrInfo.getObjectBak1());
		...
		case 2:// 转发到圈聊
			...
			if (msgType == 0) {
				...
				bundle.putLong(BundleKey.LENGTH, mrInfo.getObjectBak1());
		...
		case 3:// 转发到微信
			...
			if (api.isWXAppInstalled()) {
				...
				L.i(TAG, "onAlertItemClick text=" + text + ",shareImageUrl="
						+ shareImageUrl + ",msgType=" + msgType);
	...
	private void showdeleteDialog(final MyQMessageBean myBean) {
	...
	class DeleteMsgTask extends AsyncTask<String, Integer, MCResult> {
		private MyQMessageBean myBean;

		public DeleteMsgTask(MyQMessageBean myBean) {
			this.myBean = myBean;
		}
		...
		protected MCResult doInBackground(String... params) {
			...
			try {
				result = APIRequestServers
						.deleteMessages(c, new String[] { myBean.getMsgBean()
								.getMessageId() + "" });
			} catch (Exception e) {
				result = null;
				e.printStackTrace();
		...
		protected void onPostExecute(MCResult mcResult) {
			...
			if (null != mcResult) {
				if (mcResult.getResultCode() == 1) {
					ChatMessageBeanService.getService(c).deleteChat(
							myBean.getMsgBean().getMessageId() + "", friendId);
					...
					notifyDataSetChanged();
					try {
						if (QChatActivity.instance.friendId.equals(myBean
								.getMemberId()))
	...
	class DownLoadTask extends AsyncTask<String, Integer, String> {
		...
		protected void onPostExecute(String path) {
			...
			L.d(TAG, "DownLoadTask path=" + path);
			if (null != path) {
				playVoice(v, path);
			} else {
				// T.show(c, T.ErrStr);
	...
	private void reSendDialog(final MyQMessageBean myBean,
			final LoadQuuMessageInfo infoSet) {
