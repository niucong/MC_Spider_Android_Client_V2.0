...
public class FriendNamesAdapter<T> extends BaseAdapter implements Filterable {
	 ...
	 */
	public FriendNamesAdapter(Context context, int textViewResourceId) {
	 ...
	 *            populated
	 */
	public FriendNamesAdapter(Context context, int resource,
	...
	private View createViewFromResource(int position, View convertView,
		...
		FriendSimpleBean bean = (FriendSimpleBean) getItem(position);
		String fName = bean.getFriendName();
		if (fName != null && !"".equals(fName)) {
			text.setText(bean.getMemberName() + "<" + fName + ">");
		} else {
			text.setText(bean.getMemberName());
		}
	...
	private class ArrayFilter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence prefix) {
			...
			if (prefix == null || prefix.length() == 0) {
				...
				final ArrayList<FriendSimpleBean> newValues = new ArrayList<FriendSimpleBean>(
						count);
				for (int i = 0; i < count; i++) {
					FriendSimpleBean value = (FriendSimpleBean) values.get(i);
					String displayName = value.getMemberName().toLowerCase();
					String fName = value.getFriendName().toLowerCase();
					// 查找拼音
					if (displayName.startsWith(prefixString)
							|| fName.startsWith(prefixString)) {
						newValues.add(value);
					} else {
						boolean isStartWith = false;
						final String pyName = value.getMemberNamePY()
								.toLowerCase();
						if (pyName.contains(",")) {
							String[] pys = pyName.split(",");
							int size = 0;
							if (pys != null) {
								size = pys.length;
							...
							if (pyName.startsWith(prefixString)) {
								newValues.add(value);
								isStartWith = true;
							}
						}
						...
						if (!isStartWith) {
							final String jpName = value.getMemberNameJP()
									.toLowerCase();
							if (jpName.contains(",")) {
								String[] jps = jpName.split(",");
								int size = 0;
								if (jps != null) {
									size = jps.length;
									for (int j = 0; j < size; j++) {
								...
								if (jpName.startsWith(prefixString)) {
									newValues.add(value);
								}
							}
						}
						...
						if (!isStartWith) {
							final String pyFName = value.getFriendNamePY()
									.toLowerCase();
							if (pyFName.contains(",")) {
								String[] jps = pyFName.split(",");
								int size = 0;
								if (jps != null) {
									size = jps.length;
									for (int j = 0; j < size; j++) {
								...
								if (pyFName.startsWith(prefixString)) {
									newValues.add(value);
								}
							}
						}
						...
						if (!isStartWith) {
							final String jpFName = value.getFriendNameJP()
									.toLowerCase();
							if (jpFName.contains(",")) {
								String[] jps = jpFName.split(",");
								int size = 0;
								if (jps != null) {
									size = jps.length;
									for (int j = 0; j < size; j++) {
								...
								if (jpFName.startsWith(prefixString)) {
									newValues.add(value);
								}
							}
						}
