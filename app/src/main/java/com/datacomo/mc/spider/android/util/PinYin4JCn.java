...
public class PinYin4JCn {
	...
	/**
	 * app_g_group_info表=group
	 * 
	 * @param table
	 * @param keyword
	 * @return
	 */
	public static String convertPy(String table, String keyword) {
		String str = null;
		if ("leaguer".equals(table)) {
			str = converterToSpell(keyword);
	...
	public static String convertJp(String table, String keyword) {
		String str = null;
		if ("leaguer".equals(table)) {
			str = converterToFirstSpell(keyword);
	 ...
	 * @return
	 */
	public static String convertPyLong(String table, String keyword) {
		int count = (int) Math.ceil((double) keyword.length() / (double) 5);
		StringBuilder pinyin = new StringBuilder();
		String tempStr = "";
		for (int i = 0; i < count; i++) {
			if (i != count - 1) {
				tempStr = keyword.substring(i * 5, (i + 1) * 5);
				pinyin.append(PinYin4JCn.convertPy(table, tempStr));
			} else {
				tempStr = keyword.substring(i * 5, keyword.length());
				pinyin.append(PinYin4JCn.convertPy(table, tempStr));
			}
			if (pinyin.toString().length() > 200) {
				return pinyin.toString().length() < 200 ? pinyin.toString()
						: pinyin.toString().substring(0, 200);
			}
		}
		return pinyin.toString().length() < 200 ? pinyin.toString() : pinyin
